...
try: 
    ...
    for idx, search_term in enumerate(search_terms[start_term:end_term]):
        ...
        try:
            ...
            dbclient.rollback()
            logger.log_message(f"Database error processing search term {search_term}: {e}")
            print(f"Database error processing search term {search_term}: {e}")
    ...
    logger.log_message(f"Failed to set up database or logger: {e}")
    print(f"Failed to set up database or logger: {e}")
